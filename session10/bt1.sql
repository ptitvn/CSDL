...
-- 2) Tạo view view_users_firstname: danh sách người dùng có họ "Nguyễn"
CREATE OR REPLACE VIEW view_users_firstname AS
SELECT user_id, username, full_name, email, created_at
FROM users
WHERE SUBSTRING_INDEX(TRIM(full_name), ' ', 1) = 'Nguyễn';

-- 3) Hiển thị view vừa tạo
SELECT * FROM view_users_firstname;

-- (tuỳ chọn) xem câu lệnh tạo view
-- SHOW CREATE VIEW view_users_firstname;

-- 4) Thêm 1 user mới có họ "Nguyễn"
INSERT INTO users(username, full_name, gender, email, password, birthdate, hometown)
VALUES ('nguyen_new', 'Nguyễn Văn Mới', 'Nam', 'nguyen_new@gmail.com', '123', '1996-06-06', 'Hà Nội');

-- Truy vấn lại view để kiểm tra user mới có xuất hiện không
SELECT * FROM view_users_firstname;
...
-- Truy vấn lại view để kiểm tra user đã bị xóa còn xuất hiện không
SELECT * FROM view_users_firstname;
